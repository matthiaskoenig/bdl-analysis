...
plot_top_cluster_representatives <- function(method="ys3", Ntop=11, labels=TRUE){
  ...
  name_cluster <- sprintf("cluster.mean", k)
  for (k in 1:Ngroups){
      # YS3 in [-1,1]
      ys3 <- cluster.cor[[k]]
...
tmp <- acast(tmp, exp~pre, value.var="freq", fill=0)
par(mfrow=c(1,ncol(tmp)))
for (k in 1:ncol(tmp)){
  barplot(tmp[,k]/sum(tmp[,k]), 
  main=sprintf("Predicted in class: %s", node_classes[k]),
  ylim=c(0, 1.0),
  xlab="exp class")
}
par(mfrow=c(1,1))
...
plot_predicted_classes2 <- function(){
  ...
  for (k in 1:length(node_classes)){
    ...
    # create the bar plot
    name <- sprintf("Predicted class: %sh", node_classes[k])
    barplot(tab, beside=TRUE,
            main=name, 
            xlab="experimental time after BDL [h]", ylab="fraction of predictions",
            ylim=c(0,1), col=bar_colors) 
    if (k==1){
      legend("topright", legend=c("single factors", "double factors", 
                                  "best single gene", "mean cluster"), 
