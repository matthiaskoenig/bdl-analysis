...
``` {r ActBControlPlot, fig.width=10, fig.height=7,  htmlcap='<b>Figure Actb Control</b>: Correlation plot of the Actb probes from the 3 Fluidigm chips: Actb (fibrosis), Actb.x (ADME), Actb.y (Cytokines). The top row shows the individual time courses, the bottom row the pair wise plot of individual data points.'}
plot_actb_control()
...
rm(fil_tab)
```
Based on the adjusted p-values the data set was reduced from original 154 factors to 90. Almost all `Cytokines` genes (inflammation panel) were retained in the data set whereas many of the `ADME` and `Fibrosis` genes are filtered.
...
``` {r correlationPlots2, fig.width=10, fig.height=10, error=TRUE, htmlcap='<b>Figure Heatmap YS3:</b> Correlation matrix based on YS3 correlation.'}
...
## Correlations of histological, biochemical and antibody factors
...
get_histological_correlations <- function(method="ys3", cor.cutoff=0.6){
  ...
  hist_facs <- BDLfactors$id[anova.accept & (BDLfactors$ftype %in% c("Antibodies", "Biochemistry", "Histology"))]
  print(hist_facs)
  ...
  print(table(col.accept))  # how many factors are filtered based on correlation threshold
...
# plot in report
plot_hist_topcors(method="ys3", labels=TRUE, mfrow=c(5,2))
```
...
Here, the top cluster representatives are calculated for every cluster. The evaluation of all factors in a cluster is hereby based on the correlation between the factors and the mean cluster time course. 
...
plot_top_cluster_representatives <- function(method="ys3", Ntop=11, labels=TRUE){
  ...
  hmap_colors <- HeatmapColors()
  for (k in 1:Ngroups){
      name_cluster <- sprintf("cluster.mean.%s", k)
      ...
      v <- ys3[, which(names(dgroup)==name_cluster)]
      # sort by absolute correlation
      v.sorted <- rev(v[order(abs(v))])
...
separately to each sub-group,  and so on recursively until the subgroups either reach a minimum size (`minbucket`) or until no improvement can be made.
...
# TODO: create the matrix of how is predicted (on normal and log scale)
...
## Random double representative predictions
...
## Best factor combinations for predicting time classes
