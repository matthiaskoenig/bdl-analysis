...
To demonstrate the model fitting and model evaluation with the test data sets.
``` {r}
...
pred.time <- exp(predict(tree.reg, newdata=treedata.reg, method="anova"))-1
data.frame(BDLsamples$time, pred.time)
...
plot(BDLsamples$time, pred.time, col=rgb(0,0,1, 0.5), pch=16)
abline(a=0, b=1)

# plot the tree predictions
for (k in 1:100){
  pred.time <- exp(predict(tree.reg, newdata=res[[k]], method="anova"))-1
  points(BDLsamples$time, pred.time, col=rgb(0.5,0.5,0.5, 0.1), pch=16, cex=0.7)  
}

# make all the predictions
pred.res <- vector("list", length(res))
for (k in (1:length(res))){
  print(k)
  pred.res[[k]] <- exp(predict(tree.reg, newdata=res[[k]], method="anova"))-1
}


plot(BDLsamples$time, pred.res[[1]])
pred.single[[1]]
