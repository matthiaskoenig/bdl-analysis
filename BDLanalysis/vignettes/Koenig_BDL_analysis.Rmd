...
# mean predictions (i.e. with mean cluster data)
pred.mean <- exp(predict(tree.reg, newdata=treedata.mean, method="anova"))-1
data.frame(BDLsamples$time, pred.mean)

# plot the tree predictions
plot(BDLsamples$time, pred.mean, col=rgb(0,0,1, 0.5), pch=16)
abline(a=0, b=1)
for (k in 1:100){
  # back conversion (log)
  pred.time <- exp(predict(tree.reg, newdata=treedata.single[[k]], method="anova"))-1
...
dist.mean <- sum( (log(pred.mean+1) - log(BDLsamples$time+1))^2 )
dist.mean
