...
Decision trees are calculated using the clusters as predictors using the packages `rpart` and `rpart.plot`
``` {r}
require(rpart)
require(rpart.plot)

```{r}
# hierarchical clusters for ys3
method <- "ys3"
cor.cluster <- correlation_matrix_for_method(method)
hc <- hclust(dist(cor.cluster)) 
groups <- cutree(hc, k=Ngroups)
...
for (ks in 1:(Nt*Nr)){
  ...
  for (kgroup in 1:Ngroups){
    ...
    treedata.mean[ks, (kgroup+1)] <- mean(as.numeric(BDLdata.norm[ks, factors]), na.rm=TRUE)
  }
}
print(treedata.mean)
```
...
Ndata <- prod(group_table) *Nr*Nt  # number of group combinations * number repeats
na.vec <- rep(NA, Ndata)
...
head(treedata.single)

group_table
# Now fill the single factor data frame with data
# This should work (~450 000 * 15)
  ...
  for (n2 in facs_2){
    for (n3 in facs_3){
      for (n4 in facs_4){
        for (n5 in facs_5){
          for (n6 in facs_6){
            # do all the 40 samples for this combination
            for (k in 1:(Nr*Nt){
              # TODO: fill
              treedata.single[counter, c('class', 'c1', 'c2', 'c3', 'c4', 'c5', 'c6', 
                                         'c1.id', 'c2.id', 'c3.id', 'c4.id', 'c5.id', 'c6.id')] <- list(  ) 
              counter = counter + 1
            }
          }
        }
...
``` {r}
plot(treedata.mean[, 2:7], xlim=c(-0.5, 0.7), ylim=c(-0.5, 0.7), col=treedata.mean$class, pch=19)
```
...
print(tree.model)
# pretty plot
prp(tree.model, type=0, extra=101, yesno=TRUE)

# evaluation via prediction
...
# TODO: evaluate the tree on single factor data

```

Now analog model fitting with a regression tree.
``` {r}
# fit the tree with regression (on log scale)
# -------------------------------------------
