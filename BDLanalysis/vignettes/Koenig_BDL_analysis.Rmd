...
# TODO: put in proper function

get_histological_correlations <- function(cor.method="ys3", cor.cutoff=0.6){
  # correlation matrix
  cor_mat <- cor.matrices[[cor.method]]
...
plot_hist_topcors <- function(labels=TRUE, mfrow=c(10,1)){
  ...
  for (name in hist_facs){
    ...
    # TODO: refactor and get the numbers out
    print(v.sorted[2:11])
...
# TODO: refactor
plot_top_cluster_representatives <- function(method, Ntop=11, labels=TRUE){
  # get clusters for method
  hc.res <- hclust.res[[method]]
  groups.hc.order <- hc.res$groups.hc.order  
  ...
  for (k in 1:Ngroups){
      ...
      # TODO: put the calculation of YS & YR in methods for reusing
