...
# subset of correlation matrix (corresponding rows)
...
# Get the top correlations for every non-RNA factor with the RNA factors, 
# sorted by absolute values

png(filename=file.path(resultsPath, "correlation", "histological_top_correlation.pdf"), width=3000, height=1000, res=200)  
par(mfrow=c(5,2))
for (name in hist_facs){
  # get the elements which are not histological factors
  v <- cor.ys3[!(colnames(cor.ys3) %in% hist_facs), name]
...
# plot color key (do all plots via SVG)
svg(filename=file.path(resultsPath, "correlation", "histological_top_legend.svg"), 
    width=5, 
    height=4, 
    pointsize=8)

corrplot(matrix(seq(from=-1, to=1, length.out=50), nrow=5, ncol=10), 
         method="pie", type="full", 
         tl.cex=1.0, tl.col="black", col=col2(100), insig="p-value", sig.level=-1,
        p.mat=mv)
dev.off()
...
# Calculate the ys3 correlation between all cluster members and the cluster mean
# data
...
svg(filename=file.path(resultsPath, "correlation", "cluster_top_correlation.svg"), 
    width=5, 
    height=5, 
    pointsize=8)
...
for (k in 1:Ngroups){
    ...
    # calculate the mean over the repeats
    dgroup.mean <- bdl_mean_data(dgroup, BDLsamples)
    
    # data.frame create, now calculate the ys3 correlation on it
    ...
    v.sorted <- rev(v[order(abs(v))])
    if (length(v.sorted)>13){
      v.sorted <- v.sorted[2:13]
    } else {
      v.sorted <- v.sorted[2:length(v.sorted)]
    }
