...
# Spearman
pdf(file.path(resultsPath, "correlation", "cor.spearman.pdf"),
        width=hmap.settings$width, height=hmap.settings$height, pointsize=hmap.settings$pointsize)
f_corrplot(method="spearman")
invisible(dev.off())

# Pearson
pdf(file.path(resultsPath, "correlation", "cor.pearson.pdf"),
        width=hmap.settings$width, height=hmap.settings$height, pointsize=hmap.settings$pointsize)
f_corrplot(method="pearson")
invisible(dev.off())
```
...
# unnormalized correlations in [0, 1]
...
f_corrplot("cor.ys1", data=cor.ys1, path=resultsPath)
f_corrplot("cor.ys2", data=cor.ys2, path=resultsPath)
f_corrplot("cor.ys3", data=cor.ys3, path=resultsPath)
f_corrplot("cor.yr1", data=cor.yr1, path=resultsPath)
f_corrplot("cor.yr2", data=cor.yr2, path=resultsPath)
f_corrplot("cor.yr3", data=cor.yr3, path=resultsPath)
```
...
In a first step all factors are normalized for comparison based on their range. The normalization is performed for every factor separately via
