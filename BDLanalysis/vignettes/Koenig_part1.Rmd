...
* **factor** : one of the measured quantities/parameters over time, i.e. either 
    ...
    + one of the (immuno-)histochemical markers (e.g. BrdU-positive Kupffer cells, CTGF, S100A4)
...
``` {r echo=FALSE}
fig.cap.tc_heatmap_all <- 'Figure: Timecourse heatmap of all factors. Rows correspond to individual factors, with factor order corresponding to the original data set: `GE_ADME`, `GE_Cytokines`, `GE_Fibrosis`, `Biochemistry`, `Histology`. Columns correspond to the 40 samples with 5 subsequent samples belonging to one of the 8 time points. The data is row scaled, i.e. every individual factor is scaled to have mean zero and standard deviation one.'
```
...
get_histological_correlations <- function(method="ys3", cor.cutoff=0.6){
  ...
  hist_facs <- BDLfactors$id[anova.accept & 
                (BDLfactors$ftype %in% c("Biochemistry", "Histology"))]
...
```
``` {r plot_clusters_top_correlations, fig.width=7, fig.height=7, error=TRUE, htmlcap=fig.cap.top_clusters, fig.cap=fig.cap.top_clusters}
plot_top_cluster_representatives(labels=TRUE)
```
...
```{r echo=FALSE}
fig.cap.tree_prp <- 'Figure: Regression tree on mean clusters. Fitted regression tree on the mean cluster data, predicting the log time classes.'
```
``` {r plot_tree_prp, fig.width=7, fig.height=7, error=TRUE, htmlcap=fig.cap.tree_prp, fig.cap=fig.cap.tree.prp}
