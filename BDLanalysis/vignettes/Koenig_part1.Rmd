...
In the following steps we look at specific parts of the correlation matrix. First we are interested in the top correlations between transcripts and classical factors, i.e. biochemical, histological and immunostaining based factors. The genes are filtered based on a cutoff for the correlation, i.e. columns in which no correlation coefficient `abs(value) >= cor.cutoff` exists are filtered out. Consequently, only transcripts are retained with a absolute correlation coeffient above this threshold.
