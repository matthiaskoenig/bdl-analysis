...
Dimension reduction of the BDL data set was performed by filtering out factors which are not significantly changing over time. To analyse if a factor is changing in time an analysis of variance (ANOVA), a collection of statistical models used to analyze the differences among group means, was performed. In its simplest form, ANOVA provides a statistical test of whether or not the means of several groups are equal, and therefore generalizes the t-test to more than two groups, with the groups being the sampled time points. For every of the individual factors in the BDL data set an ANOVA was calculated. Subsequently factors were filtered based on the adjusted p-value of the ANOVA, adjusted via multiple testing corrections.
...
A multitude of tests were performed, namely an ANOVA for every single factor. Consequently, the reported p-values of the ANOVA have to be adjusted via multiple testing procedures.
